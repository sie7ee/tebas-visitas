...
//                              break;
            ...
            _createTableRows: function(tbody){
                  ...
                  if(self._querySuccess){
                        ...
                        if(self.options.paginate.scroll){
                              
                              //crear la tabla que se encargara de usar el scroll
                              var tbodyScroll = self._createTableRowsScroll(tbody);
                              self._createTableRowsLoadData(tbodyScroll);
            ...
            _createTableRowsScroll: function(tbody){
                  
                  var self = this;
                  var tr = $('<tr></tr>').appendTo(tbody);
                  var td = $('<td></td>').attr('colspan', self._aColumns.length).css({marginn:'0px', padding:'0px'}).appendTo(tr);
                  var divScroll = $('<div></div>').css({'overflow':'scroll','overflow-x':'auto','overflow-y':'auto'}).appendTo(td);
                  var tableScroll = $('<table></table>').appendTo(divScroll);
                  var body = $('<tbody></tbody>').appendTo(tableScroll);
                        ...
                        $.each(self._aDataLoad, function(keyData, valueData){
                              ...
                              $.each(self._aColumns, function(keyColumns, valueColumns){
                                    ...
                                    if(valueColumns === 'actions'){
                                          ...
                                          self._createTableRowsData(ttr, valueData[valueColumns], columns[valueColumns].width);
                                    }
                              });
                        });
            },
            ...
            _createTableRowsData: function(ttr, data, width){
                  ...
                  if(data === undefined){
                        ...
                        self._createTableTd('', ttr, width);
                                    
                  } else {
                        
                        self._createTableTd(data, ttr, width);
                  } 
            },
            ...
            _createTableTd: function(data, ttr, width){
                  
                  var widthTd = (typeof width != 'undefined') ? width : '';
                  return $('<td></td>').css({'vertical-align': 'middle', 'width':widthTd}).text(data).appendTo(ttr);
            },
